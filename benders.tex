 \documentclass[twoside]{book}
\usepackage{geometry}
\usepackage{puzzles}



\geometry%{top=0.9in, bottom=0.9in,left=0.9in, right=0.9in, paperwidth=6in, paperheight=9in}
{top=0.9in, bottom=0.9in,inner=0.9in, outer=0.7in, paperwidth=6in, paperheight=9in}


\def\thetitle{Математические умопомрачилки} %заумки???
\def\theauthor{Питер Уинклер}

\hypersetup{
pdftitle={\thetitle},
pdfauthor={\theauthor},
pdfsubject={Математика}}


\makeindex[title=Указатель головоломок,intoc]

\makeatletter
\newcommand{\rindex}[2][\imki@jobname]{%
  \index[#1]{\detokenize{#2}}%
}
\makeatother


\begin{document}
%\pagestyle{empty}

\title{\thetitle}
\author{\theauthor\\
\\
Перевод с английского большой компанией}
\date{}
\maketitle

\thispagestyle{empty}

Предварительное издание предназначенное исключительно для отлова ляпов. 

Исправления слать по адресу 
\url{petrunin@math.psu.edu}.

\vfill

\pagebreak

Настоящими авторами этого сборника являются люди со всей планеты, которые присылали мне задачи, включая многих читателей моей предыдущей книжки, а также новых знакомых и товарищей по работе из Новой Англии.
Я особо признателен учредителям профессуры Альберта Брэдли в Дартмутском колледже.
%Albert Bradley Third Century Professorship in the Sciences at Dartmouth.

Однако всё из написанного, что я могу посвятить, посвящается моим родителям,
\begin{center}
Бернарду и Мирьям Уинкерам,
\end{center}
которые должно быть мечтали видеть своего первенца полезным членом общества, пока на их глазах он становился математиком.

\thispagestyle{empty}
\input{preface.tex}
\input{warm-ups.tex}
\input{warm-ups-sol.tex}
\input{stratch.tex}
\input{stratch-sol.tex}
\input{numericals.tex}
\input{numericals-sol.tex}
\input{alice.tex}
\input{alice-sol.tex}
\input{hipe.tex}
\input{2d3d.tex}
\input{2d3d-sol.tex}
\input{1d.tex}
\input{1d-sol.tex}
\input{games.tex}
\input{games-sol.tex}
\input{old.tex}
\input{old-sol.tex}
\input{severe.tex}
\input{severe-sol.tex}
\input{unsolved.tex}

\appendix
\input{afterword.tex}
{
\small

\printindex

}

{

\sloppy

\printbibliography[heading=bibintoc]


\fussy

}

\newpage

{

\tableofcontents

}

\end{document}
